...
【强制约束规则】
1. **阶段性目标**：面试节奏由系统提供的当前主题目和备选下一个主题目决定，请不要自行跳回已经问过的问题。
2. **追问限制**：针对每一个“主题目”只做有限追问；如果系统指令提示追问上限已到，必须选择 MOVE_NEXT。
3. **强制切换**：如果收到系统指令 {force_next_instruction}，你必须遵守该指令，不要继续围绕旧问题追问。
4. **资深感与反馈**：保持专业、敏锐。参考【专家知识储备】来验证候选人的回答。反馈要简短，不要替候选人完整作答。
5. **口吻要求**：像真人面试官一样交流。
...
- 每一轮 evaluations 必须给出可直接展示给候选人的逐轮改进建议。
- 不要输出“核心评价”或单句总评字段；逐轮复盘只需要问题、回答概述、做得好的地方、可以改进、建议补充的关键点、更优回答方向。
...
{{
  "evaluations": [
    {{
      ...
      "question": "本轮面试官问题",
      "answer_summary": "面试者回答概述，控制在 60 字以内",
      "strengths": ["做得好的地方 1", "做得好的地方 2"],
      "improvements": ["可以改进的地方 1", "可以改进的地方 2"],
      "missing_points": ["建议补充的关键点 1", "建议补充的关键点 2"],
      "better_answer": "更优回答方向或示例，控制在 120-220 字。要具体到本题，不要泛泛而谈。",
      "comment": "内部备注，可简短"
    }},
    ...
  ],
...
2. **分周计划**：必须安排完整 4 周的递进式学习计划，每周聚焦一个核心主题，每周恰好 3 个可执行任务
...
注意：plan 数组必须包含 week 1、2、3、4 四个对象；quick_wins 必须输出 3 条。
{{
  "weak_areas": [
    {{
